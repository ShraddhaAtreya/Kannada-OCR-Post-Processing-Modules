...
from parallel_processor import process_words_in_batches
from domain_classifier import classify_domain, get_domain_display_text, get_domain_boost, DOMAIN_GENERAL
from test_context import clean_word
...
# Person agreement constants
FIRST_PERSON_SINGULAR  = {"ನಾನು"}
FIRST_PERSON_PLURAL    = {"ನಾವು"}
SECOND_PERSON_SINGULAR = {"ನೀನು"}
SECOND_PERSON_PLURAL   = {"ನೀವು"}
NEUTER_VERB_SUFFIXES   = ["ತ್ತದೆ", "ಯಿತು", "ಆಯಿತು", "ಗುತ್ತದೆ"]
NEUTER_TO_FIRST_SG = {
    "ತ್ತದೆ":   "ತ್ತೇನೆ",
    "ಗುತ್ತದೆ": "ಗುತ್ತೇನೆ",
    "ಯಿತು":   "ದೆ",
...
# Verb suffixes by person
# 3rd person neuter/neutral — ತ್ತದೆ, ಯಿತು, ಆಯಿತು
NEUTER_VERB_SUFFIXES = ["ತ್ತದೆ", "ಯಿತು", "ಆಯಿತು", "ಗುತ್ತದೆ"]
# 1st person singular correct suffixes — ತ್ತೇನೆ, ದೆ, ದೇನೆ
FIRST_SG_VERB_SUFFIXES = ["ತ್ತೇನೆ", "ದೆ", "ದೇನೆ", "ಏನೆ"]
# 1st person plural correct suffixes — ತ್ತೇವೆ, ದೆವು
FIRST_PL_VERB_SUFFIXES = ["ತ್ತೇವೆ", "ದೆವು", "ದೇವೆ"]
# 2nd person singular correct suffixes — ತ್ತೀಯ, ದೆ
SECOND_SG_VERB_SUFFIXES = ["ತ್ತೀಯ", "ದೆ", "ದಿಯ"]
# 2nd person plural correct suffixes — ತ್ತೀರಿ, ದಿರಿ
SECOND_PL_VERB_SUFFIXES = ["ತ್ತೀರಿ", "ದಿರಿ", "ದೀರಿ"]

# Mapping from wrong suffix → correct 1st person singular suffix
NEUTER_TO_FIRST_SG = {
    "ತ್ತದೆ":   "ತ್ತೇನೆ",
    "ಗುತ್ತದೆ": "ಗುತ್ತೇನೆ",
    "ಯಿತು":   "ದೆ",
...
# ===================================================================
# AUTO CORRECT TAB — VERSION GENERATION
# ===================================================================

def generate_auto_correct_versions(extracted_text):
    """
    Generate 5 paragraph versions with different correction strategies.
    Returns tuple: (version1_text, version1_count, version2_text, version2_count, ..., version5_text, version5_count, detected_domain)
    """
    if not extracted_text or not extracted_text.strip():
        return "", 0, "", 0, "", 0, "", 0, "", 0, "General"
    
    if not validator:
        return "", 0, "", 0, "", 0, "", 0, "", 0, "General"
    
    # Get domain for Version 5
    detected_domain = classify_domain(extracted_text)
    domain_label = get_domain_display_text(extracted_text)
    ...
    # Generate Version 1 — High Confidence Only
    v1_words = []
    v1_count = 0
    for item in word_validations:
        if item['valid'] or not item['result']:
            v1_words.append(item['original'])
        else:
            result = item['result']
            ...
            # Apply if source is in high-confidence list OR confidence >= 0.90
            if (source in ['extra_suffix_removal', 'vibhakti_correction', 'sandhi_correction']
                    or confidence >= 0.90) and suggestions:
                v1_words.append(suggestions[0])
                v1_count += 1
            else:
                v1_words.append(item['original'])
    
    v1_text = ' '.join(v1_words)
    
    # Generate Version 2 — Spelling + Morphology
    v2_words = []
    v2_count = 0
    for item in word_validations:
        if item['valid'] or not item['result']:
            v2_words.append(item['original'])
        else:
            result = item['result']
            source = result.get('source', '')
            confidence = result.get('confidence', 0.0)
            ...
            # Apply if source in list OR confidence >= 0.75
            if (source in ['extra_suffix_removal', 'vibhakti_correction', 'sandhi_correction',
                          'fst_morphology', 'ocr_direct_fix']
                    or confidence >= 0.75) and suggestions:
                v2_words.append(suggestions[0])
                v2_count += 1
            else:
                v2_words.append(item['original'])
    ...
    # Generate Version 3 — Spelling + Grammar + Postposition
    v3_words = []
    v3_count = 0
    for item in word_validations:
        if item['valid'] or not item['result']:
            v3_words.append(item['original'])
        else:
            result = item['result']
            source = result.get('source', '')
            confidence = result.get('confidence', 0.0)
            ...
            # Apply if source in list OR confidence >= 0.60
            if (source in ['extra_suffix_removal', 'vibhakti_correction', 'sandhi_correction',
                          'fst_morphology', 'ocr_direct_fix', 'postposition_correction', 'fuzzy_hybrid']
                    or confidence >= 0.60) and suggestions:
                v3_words.append(suggestions[0])
                v3_count += 1
            else:
                v3_words.append(item['original'])
    
    v3_text = ' '.join(v3_words)
    
    # Generate Version 4 — All Corrections
    v4_words = []
    v4_count = 0
    for item in word_validations:
        if item['valid'] or not item['result']:
            v4_words.append(item['original'])
        else:
            result = item['result']
            suggestions = result.get('suggestions', [])
            
            # Apply Top-1 regardless of confidence
            if suggestions:
                v4_words.append(suggestions[0])
                v4_count += 1
            else:
                v4_words.append(item['original'])
    
    v4_text = ' '.join(v4_words)
    
    # Generate Version 5 — Domain Aware (rerank with domain boost)
    v5_words = []
    v5_count = 0
    for item in word_validations:
        if item['valid'] or not item['result']:
            v5_words.append(item['original'])
        else:
            result = item['result']
            source = result.get('source', '')
            confidence = result.get('confidence', 0.0)
            ...
            # Check if meets Version 3 criteria
            if (source in ['extra_suffix_removal', 'vibhakti_correction', 'sandhi_correction',
                          'fst_morphology', 'ocr_direct_fix', 'postposition_correction', 'fuzzy_hybrid']
                    or confidence >= 0.60) and suggestions:
                
                # Rerank with domain boost
                if detected_domain != DOMAIN_GENERAL:
                    ranked = [(s, get_domain_boost(s, detected_domain)) for s in suggestions]
                    ranked.sort(key=lambda x: x[1], reverse=True)
                    top_suggestion = ranked[0][0] if ranked else suggestions[0]
                else:
                    top_suggestion = suggestions[0]
                
                v5_words.append(top_suggestion)
                v5_count += 1
            else:
                v5_words.append(item['original'])
    
    v5_text = ' '.join(v5_words)
    
    return v1_text, v1_count, v2_text, v2_count, v3_text, v3_count, v4_text, v4_count, v5_text, v5_count, domain_label
...
def sync_words_from_text(extracted_text, precompute=False):
    ...
    if precompute and validator:
        print(f"[Precompute] Computing suggestions for {len(word_data)} words (parallel)...")

        def _compute_one(item):
            try:
                return precompute_suggestions_for_word(item, extracted_text)
            except Exception as e:
                print(f"[Precompute] Error for '{item['clean']}': {e}")
                return ["(No similar words found)"]

        results = process_words_in_batches(word_data, _compute_one)

        for item, suggestions in zip(word_data, results):
            item['cached_suggestions'] = suggestions if suggestions is not None \
...
def get_suggestions_radio_from_cache(word_data, idx):
    ...
    # Check if any suggestion is honorific name correction
    has_name_correction = any(
        'Honorific — formal' in s or 'Honorific — acceptable' in s
        for s in suggestions
    )

    label = (
        "⚠️ Honorific name correction detected — Suggestions"
        if has_name_correction
        else "Suggestions (select one to apply)"
    )
...
def create_app():
    ...
    with gr.Blocks(title="Kannada OCR Correction") as app:
        ...
        with gr.Tabs():
            ...
            # TAB 2: AUTO CORRECT
            # =================================================================
            with gr.Tab("Auto Correct"):
                # SECTION 1: Input display
                gr.Markdown("### 📄 OCR Extracted Text")
                ac_input_display = gr.Textbox(
                    lines=6,
                    interactive=False,
                ...
                # SECTION 2: Generate button + status
                ac_generate_btn = gr.Button("🔄 Generate Corrections", variant="primary", size="lg")
                ac_status = gr.Markdown("Click Generate Corrections to begin.")
                
                # SECTION 3: 5 version panels in a row
                with gr.Row():
                    # Version 1
                    ac_group_1 = gr.Group()
                    with ac_group_1:
                        ac_label_1 = gr.Markdown("Version 1")
                        ac_text_1 = gr.Textbox(lines=10, interactive=False, elem_classes=["kannada-text"])
                        ac_changes_1 = gr.Markdown("")
                        ac_fluency_1 = gr.Markdown("")
                        with gr.Row():
                            ac_copy_1 = gr.Button("📋 Copy", variant="secondary")
                            ac_use_1 = gr.Button("✅ Use This Version", variant="secondary")
                    
                    # Version 2
                    ac_group_2 = gr.Group()
                    with ac_group_2:
                        ac_label_2 = gr.Markdown("Version 2")
                        ac_text_2 = gr.Textbox(lines=10, interactive=False, elem_classes=["kannada-text"])
                        ac_changes_2 = gr.Markdown("")
                        ac_fluency_2 = gr.Markdown("")
                        with gr.Row():
                            ac_copy_2 = gr.Button("📋 Copy", variant="secondary")
                            ac_use_2 = gr.Button("✅ Use This Version", variant="secondary")
                    
                    # Version 3
                    ac_group_3 = gr.Group()
                    with ac_group_3:
                        ac_label_3 = gr.Markdown("Version 3")
                        ac_text_3 = gr.Textbox(lines=10, interactive=False, elem_classes=["kannada-text"])
                        ac_changes_3 = gr.Markdown("")
                        ac_fluency_3 = gr.Markdown("")
                        with gr.Row():
                            ac_copy_3 = gr.Button("📋 Copy", variant="secondary")
                            ac_use_3 = gr.Button("✅ Use This Version", variant="secondary")
                    
                    # Version 4
                    ac_group_4 = gr.Group()
                    with ac_group_4:
                        ac_label_4 = gr.Markdown("Version 4")
                        ac_text_4 = gr.Textbox(lines=10, interactive=False, elem_classes=["kannada-text"])
                        ac_changes_4 = gr.Markdown("")
                        ac_fluency_4 = gr.Markdown("")
                        with gr.Row():
                            ac_copy_4 = gr.Button("📋 Copy", variant="secondary")
                            ac_use_4 = gr.Button("✅ Use This Version", variant="secondary")
                    
                    # Version 5
                    ac_group_5 = gr.Group()
                    with ac_group_5:
                        ac_label_5 = gr.Markdown("Version 5")
                        ac_text_5 = gr.Textbox(lines=10, interactive=False, elem_classes=["kannada-text"])
                        ac_changes_5 = gr.Markdown("")
                        ac_fluency_5 = gr.Markdown("")
                        with gr.Row():
                            ac_copy_5 = gr.Button("📋 Copy", variant="secondary")
                            ac_use_5 = gr.Button("✅ Use This Version", variant="secondary")
                
                # SECTION 4: Recommendation banner
                ac_recommendation = gr.Markdown("")
                
                # Helper function to build versions
                def on_ac_generate(text):
                    global validator
                    if not text or not text.strip():
                        empty_outputs = ["Click Generate Corrections to begin.", text]
                        empty_outputs.extend([""] * 20)  # 5 versions × 4 items each
                        return tuple(empty_outputs)
                    try:
                        # Clear entity memory and rebuild from paragraph
                        ContextAnalyzer.clear_entity_memory()
                        ContextAnalyzer.build_entity_memory_from_paragraph(text)
                        print(f"[EntityMemory] Built from paragraph: {ContextAnalyzer.entity_memory}")
                        
                        # Step 1 — validate every word fresh
                        words = text.split()
                        word_results = []
                        prev_word = None
                        for idx, word in enumerate(words):
                            ...
                            try:
                                if validator:
                                    ...
                                    print(f"[DEBUG] word='{clean}' valid={result.get('valid')} source='{result.get('source')}' conf={result.get('confidence')} sugg={result.get('suggestions',[])[:2]}")
                                    word_results.append({
                                        'original':    word,
                                        'clean':       clean,
                                        'valid':       result.get('valid', True),
                                    ...
                                    print(f"[DEBUG] validator is None — skipping '{clean}'")
                                    word_results.append({
                                        'original': word,
                                        'clean':    clean,
                                        'valid':    True,
                                ...
                                print(f"[DEBUG] EXCEPTION for '{clean}': {ex}")
                                import traceback; traceback.print_exc()
                                word_results.append({
                                    'original': word,
                                    'clean':    clean,
                        ...
                        # Step 1a — SOV sentence-level correction
                        # Split paragraph into sentences, check each for SOV violation
                        # and reorder if needed. Operates on full sentences not words.
                        try:
                            import re as _re
                            raw_sentences = _re.split(r'(?<=[.।?!])\s+', text.strip())
                            sov_corrected_sentences = []
                            sov_any = False
                            for sent in raw_sentences:
                                ...
                                for wi, sw in enumerate(sent_words):
                                    ...
                                    if not sw_clean:
                                        continue
                                    flag = validator._check_sov_violation(
                                        sw_clean, wi, sent
                                    )
                                    if flag and 'Suggested:' in flag:
                                        reordered = flag.split('Suggested:')[-1].strip()
                                        sov_corrected_sentences.append(reordered)
                                        sov_found = True
                                        sov_any = True
                                        print(f"[SOV] Reordered: '{sent}' → '{reordered}'")
                                        break
                                if not sov_found:
                                    sov_corrected_sentences.append(sent)

                            if sov_any:
                                # Rebuild paragraph with SOV-corrected sentences
                                text = ' '.join(sov_corrected_sentences)
                                # Rebuild word_results from corrected text
                                words = text.split()
                                word_results = []
                                prev_word = None
                                for idx, word in enumerate(words):
                        ...
                        except Exception as sov_ex:
                            print(f"[SOV] Error: {sov_ex}")

                        # Step 1b — Pronoun consistency + Gender agreement per sentence

                        # Get dominant entity gender from entity memory
                        # Only trust KNOWN_ENTITIES — filter out common words
                        PRONOUN_WORDS = {
                            'ಅವನು', 'ಈತನು', 'ಇವನು',
                            'ಅವಳು', 'ಈಕೆ', 'ಇವಳು',
                            'ಅವರು', 'ಇವರು', 'ತಾವು'
                        }
                        # Extract proper nouns from SUBJECT_PRONOUNS
                        # (excludes pronoun words — only named entities)
                        try:
                            _known = {
                                k for k, v in ContextAnalyzer.SUBJECT_PRONOUNS.items()
                                if k not in PRONOUN_WORDS
                                and v.get('agreement', '').startswith('3SG')
                                and v.get('gender') in ('MALE', 'FEMALE')
                            }
                        except Exception:
                            _known = set()
                        dominant_gender = None
                        for entity, gender in ContextAnalyzer.entity_memory.items():
                            if entity in _known and entity not in PRONOUN_WORDS:
                                dominant_gender = gender
                                break
                        print(f"[EntityMemory] dominant_gender={dominant_gender} known_found={[e for e in ContextAnalyzer.entity_memory if e in _known]}")

                        def get_sentence_groups(word_results):
                            groups = []
                            current = []
                            for wr in word_results:
                                current.append(wr)
                                if wr['original'].strip() in ['.', '।', '?', '!']:
                        ...
                        sentence_groups = get_sentence_groups(word_results)

                        for group in sentence_groups:
                            # Step A — Pronoun consistency using entity memory
                            if dominant_gender:
                                for wr in group:
                                    w = wr['clean']
                                    if not wr['valid'] or wr['suggestions']:
                                        continue
                                    if dominant_gender == 'MALE' and w in FEMALE_PRONOUNS:
                                        wr['valid'] = False
                                        wr['source'] = 'pronoun_consistency'
                                        wr['confidence'] = 0.88
                                        wr['suggestions'] = ['ಅವನು']
                                        print(f"[Pronoun] '{w}' conflicts with dominant MALE → suggest 'ಅವನು'")
                                    elif dominant_gender == 'FEMALE' and w in MALE_PRONOUNS:
                                        wr['valid'] = False
                                        wr['source'] = 'pronoun_consistency'
                                        wr['confidence'] = 0.88
                                        wr['suggestions'] = ['ಅವಳು']
                                        print(f"[Pronoun] '{w}' conflicts with dominant FEMALE → suggest 'ಅವಳು'")

                            # Step B — Per-sentence gender and person agreement
                            # Use corrected pronoun from Step A if available
                            sent_subject_gender = None
                            sent_subject_person = None  # '1SG','1PL','2SG','2PL'
                            for wr in group:
                                if (not wr['valid'] and wr['source'] == 'pronoun_consistency'
                                        and wr['suggestions']):
                                    w = wr['suggestions'][0]
                                else:
                                    w = wr['clean']
                                if w in FIRST_PERSON_SINGULAR:
                                    sent_subject_person = '1SG'
                                    break
                                elif w in FIRST_PERSON_PLURAL:
                                    sent_subject_person = '1PL'
                                    break
                                elif w in SECOND_PERSON_SINGULAR:
                                    sent_subject_person = '2SG'
                                    break
                                elif w in SECOND_PERSON_PLURAL:
                                    sent_subject_person = '2PL'
                                    break
                                elif w in FEMALE_PRONOUNS:
                                    sent_subject_gender = 'FEMALE'
                                    break
                                elif w in MALE_PRONOUNS:
                                    sent_subject_gender = 'MALE'
                                    break

                            if not sent_subject_gender and not sent_subject_person and dominant_gender:
                                sent_subject_gender = dominant_gender

                            if not sent_subject_gender and not sent_subject_person:
                                continue
                            ...
                            for wr in group:
                                ...
                                # Person agreement — 1st person singular
                                if sent_subject_person == '1SG':
                                    corrected = None
                                    for wrong_sfx, correct_sfx in NEUTER_TO_FIRST_SG.items():
                                        if w.endswith(wrong_sfx):
                                            corrected = w[:-len(wrong_sfx)] + correct_sfx
                                            break
                                    if not corrected:
                                        for ms in MALE_VERB_SUFFIXES:
                                            if w.endswith(ms):
                                                corrected = w[:-len(ms)] + 'ತ್ತೇನೆ'
                                                break
                                    if not corrected:
                                        for fs in FEMALE_VERB_SUFFIXES:
                                            if w.endswith(fs):
                                                corrected = w[:-len(fs)] + 'ತ್ತೇನೆ'
                                                break
                                    if corrected and corrected != w:
                                        wr['valid'] = False
                                        wr['source'] = 'person_agreement'
                                        wr['confidence'] = 0.87
                                        wr['suggestions'] = [corrected]
                                        print(f"[Person] '{w}' subject=1SG → suggest '{corrected}'")

                                # Gender agreement — 3rd person
                                elif sent_subject_gender == 'FEMALE' and any(
                                    w.endswith(s) for s in MALE_VERB_SUFFIXES
                                ):
                                    female_form = None
                                    for ms, fs in zip(MALE_VERB_SUFFIXES, FEMALE_VERB_SUFFIXES):
                                        if w.endswith(ms):
                                            female_form = w[:-len(ms)] + fs
                                            break
                                    if female_form:
                                        wr['valid'] = False
                                        wr['source'] = 'gender_agreement'
                                        wr['confidence'] = 0.85
                                        wr['suggestions'] = [female_form]
                                        print(f"[Gender] '{w}' subject=FEMALE → suggest '{female_form}'")
                                elif sent_subject_gender == 'MALE' and any(
                                    w.endswith(s) for s in FEMALE_VERB_SUFFIXES
                                ):
                                    male_form = None
                                    for fs, ms in zip(FEMALE_VERB_SUFFIXES, MALE_VERB_SUFFIXES):
                                        if w.endswith(fs):
                                            male_form = w[:-len(fs)] + ms
                                            break
                                    if male_form:
                                        wr['valid'] = False
                                        wr['source'] = 'gender_agreement'
                                        wr['confidence'] = 0.85
                                        wr['suggestions'] = [male_form]
                                        print(f"[Gender] '{w}' subject=MALE → suggest '{male_form}'")

                        # Helper — pick correction based on threshold
                        def pick(item, sources, min_conf):
                            if item['valid']:
                            ...
                            if (item['source'] in sources
                                    or item['confidence'] >= min_conf):
                                return sugg[0]
                            return item['original']

                        # Helper — compute changes
                        def compute_changes(words_out):
                            changes = [
                                f"{wr['original']} -> {corrected}"
                                for wr, corrected in zip(word_results, words_out)
                                if wr['original'] != corrected
                            ]
                            return "Changes: " + ", ".join(changes) if changes else "Changes: None"

                        # Step 2a — Build rule-based versions (V2-V5) synchronously
                        V5_SOURCES = [
                            'extra_suffix_removal',
                            'vibhakti_correction',
                            'sandhi_correction',
                            'ocr_noise_list',
                            'gender_agreement',
                        ...
                        V4_SOURCES = V5_SOURCES + [
                            'fst_morphology',
                            'ocr_direct_fix',
                            'postposition_correction',
                        ]
                        V4_FULL_SOURCES = V4_SOURCES + [
                            'postposition_correction',
                            'fuzzy_hybrid',
                        ]
                        
                        # V5 — Safe Corrections Only (high confidence only)
                        v5_words = [pick(i, V5_SOURCES, 0.90) for i in word_results]
                        v5_text = ' '.join(v5_words)
                        v5_changes = compute_changes(v5_words)
                        
                        # V4 — Grammar and Spelling Correction
                        v4_words = [pick(i, V4_FULL_SOURCES, 0.60) for i in word_results]
                        v4_text = ' '.join(v4_words)
                        v4_changes = compute_changes(v4_words)
                        
                        # V3 — Domain-Aware Correction with reranking
                        detected_domain = classify_domain(text)
                        V3_SOURCES = V4_FULL_SOURCES  # Same sources as V4
                        v3_words = []
                        for item in word_results:
                            if item['valid']:
                                v3_words.append(item['original'])
                            elif not item['suggestions']:
                                v3_words.append(item['original'])
                            elif item['source'] not in V3_SOURCES or item['confidence'] < 0.60:
                                v3_words.append(item['original'])
                            else:
                                # Apply domain-aware reranking if domain detected
                                candidates = item['suggestions'][:3]
                                if detected_domain != DOMAIN_GENERAL:
                                    best_cand = candidates[0]
                                    best_boost = get_domain_boost(best_cand, detected_domain)
                                    for cand in candidates[1:]:
                                        boost = get_domain_boost(cand, detected_domain)
                                        if boost > best_boost:
                                            best_boost = boost
                                            best_cand = cand
                                    v3_words.append(best_cand)
                                else:
                                    v3_words.append(candidates[0])
                        v3_text = ' '.join(v3_words)
                        v3_changes = compute_changes(v3_words)
                        
                        # V2 — Full Rule-Based Correction (all corrections regardless of source/confidence)
                        v2_words = [
                            (i['suggestions'][0]
                             if not i['valid'] and i['suggestions']
                             else i['original'])
                            for i in word_results
                        ]
                        v2_text = ' '.join(v2_words)
                        v2_changes = compute_changes(v2_words)
                        
                        # Step 2b — Build V1 using Gemini API (parallel execution)
                        def build_gemini_v1():
                            if not GEMINI_AVAILABLE:
                                return "", "Version 1 — AI Model Full Correction (Not Available)"
                            try:
                                prompt = (
                                    "You are a Kannada OCR post-processing expert.\n"
                                    "The following paragraph was extracted from a scanned Kannada\n"
                                    "document and contains OCR errors.\n"
                                    "Correct ONLY the errors — do not rephrase, do not change\n"
                                    "meaning, do not translate.\n"
                                    "Return only the corrected Kannada paragraph, nothing else.\n"
                                    f"Paragraph: {text}"
                                )
                                response = _gemini_model.generate_content(prompt)
                                v1_text = response.text.strip() if response and response.text else ""
                                return v1_text, None
                            except Exception as e:
                                return "", f"Version 1 — AI Model Full Correction (Error: {str(e)[:40]}...)"
                        
                        # Run Gemini and rule-based versions in parallel
                        import concurrent.futures
                        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                            v1_future = executor.submit(build_gemini_v1)
                            # Rule-based are already computed synchronously above
                            v1_text, v1_label_error = v1_future.result()
                        
                        # Compute V1 label and changes
                        if v1_label_error:
                            v1_label = v1_label_error
                            v1_changes = "Changes: None"
                        else:
                            v1_words_padded = v1_text.split()
                            if len(v1_words_padded) == len([w['original'] for w in word_results]):
                                v1_changes = compute_changes(v1_words_padded)
                                c1 = sum(1 for orig, corr in zip([w['original'] for w in word_results], v1_words_padded) if orig != corr)
                                v1_label = f"Version 1 — AI Model Full Correction ({c1} corrections)"
                            else:
                                v1_changes = "Changes: AI Model correction applied"
                                c1 = 0
                                v1_label = f"Version 1 — AI Model Full Correction (word count differs)"
                        
                        # Count corrections for other versions
                        c2 = sum(1 for wr, corr in zip(word_results, v2_words) if wr['original'] != corr)
                        c3 = sum(1 for wr, corr in zip(word_results, v3_words) if wr['original'] != corr)
                        c4 = sum(1 for wr, corr in zip(word_results, v4_words) if wr['original'] != corr)
                        c5 = sum(1 for wr, corr in zip(word_results, v5_words) if wr['original'] != corr)
                        
                        # Update labels for V2-V5 with counts
                        v2_label = f"Version 2 — Full Rule-Based Correction ({c2} corrections)"
                        v3_label = f"Version 3 — Domain-Aware Correction ({c3} corrections)"
                        v4_label = f"Version 4 — Grammar and Spelling ({c4} corrections)"
                        v5_label = f"Version 5 — Safe Corrections Only ({c5} corrections)"
                        
                        # Step 3 — IndicBERT fluency scoring
                        fluency = [""] * 5
                        recommendation = "Fluency scoring not available"
                        try:
                            from indicbert_fluency_scorer import (
                                score_paragraph_versions,
                                is_ready as indicbert_ready
                            )
                            if indicbert_ready():
                                scored = score_paragraph_versions(
                                    [v1_text, v2_text, v3_text, v4_text, v5_text]
                                )
                                for r in scored:
                                    idx2 = r['version_idx']
                                    fluency[idx2] = (
                                        f"Fluency: {r['fluency']:.2f} | PPL: {r['ppl']:.1f}"
                                    )
                                best = scored[0]
                                recommendation = (
                                    f"Recommended: Version {best['version_idx']+1} — "
                                    f"highest fluency score ({best['fluency']:.2f})"
                                )
                        except Exception as e:
                            recommendation = f"Fluency scoring error: {str(e)[:60]}"
                        
                        return (
                            "Generated 5 versions successfully", text,
                            v1_label, v1_text, v1_changes, fluency[0],
                            v2_label, v2_text, v2_changes, fluency[1],
                            v3_label, v3_text, v3_changes, fluency[2],
                            v4_label, v4_text, v4_changes, fluency[3],
                            v5_label, v5_text, v5_changes, fluency[4],
                            recommendation
                        )
                        ...
                        import traceback
                        traceback.print_exc()
                        empty_outputs = [f"Error: {str(e)}", text]
                        empty_outputs.extend([""] * 20)  # 5 versions × 4 items each
                        return tuple(empty_outputs)
                
                ac_generate_btn.click(
                    fn=on_ac_generate,
                    inputs=[extracted_text],
                    outputs=[ac_status, ac_input_display,
                             ac_label_1, ac_text_1, ac_changes_1, ac_fluency_1,
                             ac_label_2, ac_text_2, ac_changes_2, ac_fluency_2,
                             ac_label_3, ac_text_3, ac_changes_3, ac_fluency_3,
                             ac_label_4, ac_text_4, ac_changes_4, ac_fluency_4,
                             ac_label_5, ac_text_5, ac_changes_5, ac_fluency_5,
                             ac_recommendation]
                )
                
                # Copy buttons
                def on_copy(text, ver_num):
                    if text and text.strip():
                        gr.Info(f"Version {ver_num} copied to clipboard")
                
                ac_copy_1.click(fn=on_copy, inputs=[ac_text_1, gr.State(1)])
                ac_copy_2.click(fn=on_copy, inputs=[ac_text_2, gr.State(2)])
                ac_copy_3.click(fn=on_copy, inputs=[ac_text_3, gr.State(3)])
                ac_copy_4.click(fn=on_copy, inputs=[ac_text_4, gr.State(4)])
                ac_copy_5.click(fn=on_copy, inputs=[ac_text_5, gr.State(5)])
                
                # Use This Version buttons
                def on_use(text, ver_num):
                    if text and text.strip():
                        return text, f"✅ Version {ver_num} loaded into editor — go to Correct Words tab"
                    return "", f"❌ No text in Version {ver_num}"
                
                ac_use_1.click(fn=on_use, inputs=[ac_text_1, gr.State(1)], outputs=[extracted_text, ac_status])
                ac_use_2.click(fn=on_use, inputs=[ac_text_2, gr.State(2)], outputs=[extracted_text, ac_status])
                ac_use_3.click(fn=on_use, inputs=[ac_text_3, gr.State(3)], outputs=[extracted_text, ac_status])
                ac_use_4.click(fn=on_use, inputs=[ac_text_4, gr.State(4)], outputs=[extracted_text, ac_status])
                ac_use_5.click(fn=on_use, inputs=[ac_text_5, gr.State(5)], outputs=[extracted_text, ac_status])
    ...
    # Gender mismatch check
                    if word in MALE_PRONOUNS or word in FEMALE_PRONOUNS:
                        ...
                        # Honorific check — verb AND name form
                        try:
                            if full_sentence:
                                ...
                                if h.correction_made:
                                    ...
                                    for o, c in zip(orig, corr):
                                        if o.strip('.,!?;:()[]{}"\'।') == word and o != c:
                                            suggestions_list.insert(
                                            0, f"{c} (Honorific correction)"
                                             )
                                            break

                            # NEW — Check name form correction
                            if h.subject_name_correction:
                                orig_name, formal, informal = h.subject_name_correction
                                clean_word = word.strip('.,!?;:()[]{}"\'।')
                                if clean_word == orig_name:
                                # Insert both options at top of suggestions
                                    suggestions_list.insert(0, f"{informal} (Honorific — acceptable)")
                                    suggestions_list.insert(0, f"{formal} (Honorific — formal)")
                        except Exception as e:
                            print(f"[Honorific] Error: {e}")
                ...
                def on_load_words(extracted_text_val, filter_type):
                    ...
                    # Domain classification — once per paragraph
                    domain_label = get_domain_display_text(extracted_text_val)
                    detected_domain, domain_conf, _ = classify_domain(extracted_text_val)

                    # Store detected domain in validator so ranking boost applies
                    if validator:
                        validator.detected_domain    = detected_domain
                        validator.domain_confidence  = domain_conf

                    # Append domain line to context panel text
                    context_text = domain_label + "\n\n" + context_text
